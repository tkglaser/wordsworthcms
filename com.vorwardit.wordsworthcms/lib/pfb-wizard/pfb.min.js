...
module.run(['$templateCache', function($templateCache) {
  $templateCache.put('/views/Booking.html',
    '<div class="row"><div class="col-md-12"><div class="panel panel-default"><div class="panel-heading"><h1 class="panel-title">Vielen Dank, Ihre Fahrt ist jetzt gebucht!</h1></div><div class="panel-body"><p class="pull-right hidden-print" role="group"><button class="btn btn-primary btn-xs" type="button" ng-click="ctrl.showMailForm()"><span class="fa fa-mail-forward"></span> Per Mail senden...</button> <button class="btn btn-info btn-xs" type="button" ng-click="ctrl.print()"><span class="fa fa-print"></span> Ausdrucken</button></p><p ng-if="ctrl.request.status == \'Booked\' && ctrl.request.outstanding > 0">Ihnen sollte in den n&auml;chsten Minuten eine Best&auml;tigung per EMail zugehen. Der Rechnungsbetrag in Höhe von <strong>{{ctrl.request.outstanding | currency}}</strong> wird erst fällig, wenn wir Ihnen den Auftrag verbindlich bestätigt haben. Dazu erhalten Sie von uns eine separate Auftragsbestätigung.</p><p ng-if="ctrl.request.status == \'Booked\' && ctrl.request.outstanding == 0">Ihnen sollte in den n&auml;chsten Minuten eine Best&auml;tigung per EMail zugehen. Bitte beachten Sie, dass der Vertrag erst mit Zusendung der Auftragsbestätigung zustande kommt.</p><div class="clearfix"></div><div class="panel panel-default pull-right ng-cloak" ng-show="ctrl.isMailFormShown" style="max-width:300px;"><div class="panel-body"><div ng-hide="ctrl.mailSendSuccess" class="input-group input-group-sm"><input type="text" class="form-control input-sm" placeholder="Email" ng-model="ctrl.emailForSending"> <span class="input-group-btn"><button class="btn btn-primary btn-sm" type="button" ng-disabled="ctrl.emailBeingSent" ng-click="ctrl.sendMail()"><span ng-hide="ctrl.emailBeingSent"><span class="fa fa-envelope"></span> Senden</span> <span ng-show="ctrl.emailBeingSent"><span class="fa fa-fw fa-cog fa-spin"></span></span></button></span></div><div ng-show="ctrl.mailSendSuccess" class="text-success">Email erfolgreich versandt!</div></div></div><div class="clearfix"></div><div class="panel" ng-class="(ctrl.request.outstanding > 0) ? \'panel-warning\' : \'panel-success\'"><div class="panel-heading"><h2 class="panel-title">Ihre Buchung bei PAUL f&auml;hrt Bus. Nummer: <strong>{{ctrl.request.number}}</strong></h2></div><div class="panel-body"><div class="row"><div class="col-sm-5"><h4>Rechnungsadresse</h4><address><span ng-if="ctrl.request.organisation"><strong>{{ctrl.request.organisation}}</strong><br>z.Hd.</span> <strong>{{ctrl.request.name}}</strong><br>{{ctrl.request.address}}<br>{{ctrl.request.postcode}} {{ctrl.request.town}}</address><h4>Kontakt</h4><div>Email: <strong>{{ctrl.request.email}}</strong></div></div><div class="col-sm-7"><h4>Reisedaten</h4><div class="row"><div class="col-xs-2">Fahrt:</div><div class="col-xs-10">Von <strong>{{ctrl.request.startName}}</strong> nach <strong>{{ctrl.request.targetName}}</strong></div><div class="col-xs-offset-2 col-xs-10" ng-show="ctrl.request.busShouldStay"><i class="fa fa-info-circle"></i> Bus bleibt vor Ort</div><div class="col-xs-2">Hinfahrt:</div><div class="col-xs-10">Am <strong>{{ctrl.request.startDate | date}}</strong>, <span ng-show="ctrl.request.startTimeMode == \'Departure\'">Abfahrt <strong>{{ctrl.request.startTimeDepart}}</strong> (Ankunft ca. {{ctrl.request.startTimeArrive}})</span> <span ng-show="ctrl.request.startTimeMode == \'Arrival\'">Ankunft <strong>{{ctrl.request.startTimeArrive}}</strong> (Abfahrt ca. {{ctrl.request.startTimeDepart}})</span></div><div class="col-xs-2">R&uuml;ckfahrt:</div><div class="col-xs-10"><span ng-show="ctrl.request.tripType == \'OneWay\'"><strong>Keine R&uuml;ckfahrt</strong></span> <span ng-show="ctrl.request.tripType == \'Return\'">Am <strong>{{ctrl.request.returnDate | date}}</strong>, <span ng-show="ctrl.request.returnTimeMode == \'Departure\'">R&uuml;ckfahrt <strong>{{ctrl.request.returnTimeDepart}}</strong> (R&uuml;ckkunft ca. {{ctrl.request.returnTimeArrive}})</span> <span ng-show="ctrl.request.returnTimeMode == \'Arrival\'">R&uuml;ckkunft <strong>{{ctrl.request.returnTimeArrive}}</strong> (R&uuml;ckfahrt ca. {{ctrl.request.returnTimeDepart}})</span></span></div><div class="col-xs-2">Personen:</div><div class="col-xs-10"><strong>{{ctrl.request.passengers}}</strong></div></div></div></div></div><table class="table table-responsive table-striped table-bordered table-condensed"><tr><th>Leistung</th><th class="text-center">Beschreibung</th><th class="text-right">Preis</th></tr><tr><td><p style="font-weight:bold; font-size:large">{{ctrl.request.productName}}</p></td><td><div class="text-center" ng-bind-html="ctrl.request.safeProductDescription"></div></td><td class="text-right">{{ctrl.request.price | currency}}</td></tr><tr><td class="text-right" colspan="2"><strong>Zwischensumme:</strong></td><td class="text-right">{{ctrl.request.price | currency}}</td></tr><tr><td class="text-right" colspan="2">enthalten 19% MwSt.:</td><td class="text-right">{{ctrl.request.price * 0.19 | currency}}</td></tr><tr><td class="text-right" colspan="2"><strong>Gesamtpreis inkl. MwSt.:</strong></td><td class="text-right"><strong>{{ctrl.request.price | currency}}</strong></td></tr><tr><td class="text-right" colspan="2">Bereits gezahlt:</td><td class="text-right">{{ctrl.request.paid | currency}}</td></tr><tr><td class="text-right" colspan="2">Differenz:</td><td class="text-right">{{ctrl.request.outstanding | currency}}</td></tr></table></div></div></div></div></div>');
}]);
})();
...
module.run(['$templateCache', function($templateCache) {
  $templateCache.put('/views/Order.html',
    '<div class="row"><div class="col-md-12"><div class="panel panel-primary ng-cloak" ng-show="!ctrl.redirecting"><div class="panel-heading"><h1 class="panel-title">Ihr Angebot von PAUL f&auml;hrt Bus</h1></div><div class="panel-body"><div class="row"><div class="col-sm-4"><div class="panel panel-default"><div class="panel-heading"><h4 div="" class="panel-title text-center">{{ctrl.offer.productName}}</h4></div><div class="panel-body"><h2 class="text-center"><strong>{{ctrl.offer.price / ctrl.request.passengers | currency}}</strong>/p.P.</h2><p class="text-center"><small>Gesamt {{ctrl.offer.price | currency}}</small></p><hr><div class="text-center" ng-bind-html="ctrl.offer.safeProductDescription"></div></div></div></div><div class="col-sm-8"><div class="panel panel-default"><div class="panel-body"><h4>Reisedaten <small><a href="#/request?r={{ctrl.requestId}}&s=o"><span class="fa fa-pencil"></span> Bearbeiten</a></small></h4><div class="row"><div class="col-xs-2">Fahrt:</div><div class="col-xs-10">Von <strong>{{ctrl.request.startName}}</strong> nach <strong>{{ctrl.request.targetName}}</strong></div><div class="col-xs-offset-2 col-xs-10" ng-show="ctrl.request.busShouldStay"><i class="fa fa-info-circle"></i> Bus bleibt vor Ort</div><div class="col-xs-2">Hinfahrt:</div><div class="col-xs-10">Am <strong>{{ctrl.request.startDate | date}}</strong>, <span ng-show="ctrl.request.startTimeMode == \'Departure\'">Abfahrt <strong>{{ctrl.request.startTimeDepart}}</strong> (Ankunft ca. {{ctrl.request.startTimeArrive}})</span> <span ng-show="ctrl.request.startTimeMode == \'Arrival\'">Ankunft <strong>{{ctrl.request.startTimeArrive}}</strong> (Abfahrt ca. {{ctrl.request.startTimeDepart}})</span></div><div class="col-xs-2">R&uuml;ckfahrt:</div><div class="col-xs-10"><span ng-show="ctrl.request.tripType == \'OneWay\'"><strong>Keine R&uuml;ckfahrt</strong></span> <span ng-show="ctrl.request.tripType == \'Return\'">Am <strong>{{ctrl.request.returnDate | date}}</strong>, <span ng-show="ctrl.request.returnTimeMode == \'Departure\'">R&uuml;ckfahrt <strong>{{ctrl.request.returnTimeDepart}}</strong> (R&uuml;ckkunft ca. {{ctrl.request.returnTimeArrive}})</span> <span ng-show="ctrl.request.returnTimeMode == \'Arrival\'">R&uuml;ckkunft <strong>{{ctrl.request.returnTimeArrive}}</strong> (R&uuml;ckfahrt ca. {{ctrl.request.returnTimeDepart}})</span></span></div><div class="col-xs-2">Personen:</div><div class="col-xs-10"><strong>{{ctrl.request.passengers}}</strong></div></div></div></div><h4>Ihre Rechnungsadresse</h4><div class="form-horizontal form-condensed" ng-form="ctrl.form"><div class="form-group"><label class="col-sm-3 col-md-2">Name *</label><div class="col-sm-9 col-md-10"><input type="text" class="form-control" name="name" placeholder="Ihr Name" ng-model="ctrl.name" required=""></div></div><div class="form-group"><label class="col-sm-3 col-md-2">Email *</label><div class="col-sm-9 col-md-10"><input type="email" class="form-control" name="email" placeholder="Ihre Email" ng-model="ctrl.email" required=""></div></div><div class="form-group"><label class="col-sm-3 col-md-2">Organisation</label><div class="col-sm-9 col-md-10"><input type="text" class="form-control" name="organisation" placeholder="Ihre Firma oder Arbeitgeber" ng-model="ctrl.organisation"></div></div><div class="form-group"><label class="col-sm-3 col-md-2">Strasse *</label><div class="col-sm-9 col-md-10"><input type="text" class="form-control" name="street" ng-model="ctrl.street" placeholder="Ihre Strasse" required=""></div></div><div class="form-group"><label class="col-sm-3 col-md-2">Stadt *</label><div class="col-sm-9 col-md-10"><input type="text" class="form-control" name="town" ng-model="ctrl.town" placeholder="Ihre Stadt" required=""></div></div><div class="form-group"><label class="col-sm-3 col-md-2">Postleitzahl&nbsp;*</label><div class="col-sm-9 col-md-10"><input type="text" class="form-control" name="postcode" ng-model="ctrl.postcode" placeholder="Ihre Postleitzahl" required=""></div></div><div class="form-group"><label class="col-sm-3 col-md-2">Zahlungsart</label><div class="col-sm-9 col-md-10"><div class="radio"><label><input type="radio" ng-model="ctrl.paymode" value="later"> Sp&auml;ter bezahlen</label><div ng-show="ctrl.paymode == \'later\'" class="animate-show" style="margin-left:25px"><small>Buchen sie jetzt und bezahlen sie bis zu 7 Tage vor Ihrer Reise</small></div></div><div class="radio"><label><input type="radio" ng-model="ctrl.paymode" value="now"> Jetzt bezahlen</label><div ng-show="ctrl.paymode == \'now\'" class="animate-show" style="margin-left:25px"><small>Zahlen sie jetzt sofort per <img src="https://www.paypal.com/de_DE/i/logo/PayPal_mark_37x23.gif"> oder Lastschrift</small></div></div></div></div><div class="form-group"><div class="col-sm-12"><button type="button" class="btn btn-success btn-block btn-lg" ng-disabled="!ctrl.canSubmit()" ng-click="ctrl.submit()">Weiter <span class="fa fa-chevron-right"></span></button></div></div></div></div></div></div></div></div></div>');
}]);
})();
...
module.run(['$templateCache', function($templateCache) {
  $templateCache.put('/views/OrderConfirm.html',
    '<div class="row"><div class="col-md-12"><div class="panel panel-primary ng-cloak" ng-show="!ctrl.redirecting"><div class="panel-heading"><h1 class="panel-title">Bestellung aufgeben</h1></div><div class="panel-body"><p>Vor dem Absenden Ihrer Bestellung haben Sie hier die M&ouml;glichkeit, die von Ihnen eingegebenen Daten und Leistungen zu korrigieren. Den Bestellvorgang k&ouml;nnen Sie jederzeit durch Schlie&szlig;en des Browser-Fensters beenden oder durch Dr&uuml;cken des Bestell-Buttons abschlie&szlig;en.</p><div class="row"><div class="col-sm-5"><h4>Rechnungsadresse <small><a href="#/order/{{ctrl.requestId}}/{{ctrl.offerId}}"><span class="fa fa-pencil"></span> Bearbeiten</a></small></h4><address><span ng-if="ctrl.request.organisation"><strong>{{ctrl.request.organisation}}</strong><br>z.Hd.</span> <strong>{{ctrl.request.name}}</strong><br>{{ctrl.request.address}}<br>{{ctrl.request.postcode}} {{ctrl.request.town}}</address><h4>Kontakt <small><a href="#/order/{{ctrl.requestId}}/{{ctrl.offerId}}"><span class="fa fa-pencil"></span> Bearbeiten</a></small></h4><div>Email: <strong>{{ctrl.request.email}}</strong></div></div><div class="col-sm-7"><h4>Reisedaten <small><a href="#/request?r={{ctrl.requestId}}&s=o"><span class="fa fa-pencil"></span> Bearbeiten</a></small></h4><div class="row" style="margin-bottom:18px;"><div class="col-xs-2">Fahrt:</div><div class="col-xs-10">Von <strong>{{ctrl.request.startName}}</strong> nach <strong>{{ctrl.request.targetName}}</strong></div><div class="col-xs-offset-2 col-xs-10" ng-show="ctrl.request.busShouldStay"><i class="fa fa-info-circle"></i> Bus bleibt vor Ort</div><div class="col-xs-2">Hinfahrt:</div><div class="col-xs-10">Am <strong>{{ctrl.request.startDate | date}}</strong>, <span ng-show="ctrl.request.startTimeMode == \'Departure\'">Abfahrt <strong>{{ctrl.request.startTimeDepart}}</strong> (Ankunft ca. {{ctrl.request.startTimeArrive}})</span> <span ng-show="ctrl.request.startTimeMode == \'Arrival\'">Ankunft <strong>{{ctrl.request.startTimeArrive}}</strong> (Abfahrt ca. {{ctrl.request.startTimeDepart}})</span></div><div class="col-xs-2">R&uuml;ckfahrt:</div><div class="col-xs-10"><span ng-show="ctrl.request.tripType == \'OneWay\'"><strong>Keine R&uuml;ckfahrt</strong></span> <span ng-show="ctrl.request.tripType == \'Return\'">Am <strong>{{ctrl.request.returnDate | date}}</strong>, <span ng-show="ctrl.request.returnTimeMode == \'Departure\'">R&uuml;ckfahrt <strong>{{ctrl.request.returnTimeDepart}}</strong> (R&uuml;ckkunft ca. {{ctrl.request.returnTimeArrive}})</span> <span ng-show="ctrl.request.returnTimeMode == \'Arrival\'">R&uuml;ckkunft <strong>{{ctrl.request.returnTimeArrive}}</strong> (R&uuml;ckfahrt ca. {{ctrl.request.returnTimeDepart}})</span></span></div><div class="col-xs-2">Personen:</div><div class="col-xs-10"><strong>{{ctrl.request.passengers}}</strong></div></div><h4>Zahlungsweise <small><a href="#/order/{{ctrl.requestId}}/{{ctrl.offerId}}"><span class="fa fa-pencil"></span> Bearbeiten</a></small></h4><div ng-show="ctrl.paymode == \'later\'"><strong>Sp&auml;ter bezahlen</strong></div><div ng-hide="ctrl.paymode == \'later\'"><strong>Jetzt bezahlen</strong></div></div></div><p><div class="checkbox" ng-class="ctrl.acceptedStyle()"><label><input type="checkbox" ng-model="ctrl.accepted"> Ich habe die <a href="http://static.paulfaehrtbus.de/agb" target="_blank">AGBs <span class="glyphicon glyphicon-new-window"></span></a> der Magnitude Internet GmbH gelesen und erkl&auml;re hiermit mein Einverst&auml;ndnis.</label></div></p><table class="table table-responsive table-striped table-bordered table-condensed"><tr><th>Leistung</th><th class="text-center">Beschreibung</th><th class="text-right">Preis</th></tr><tr><td><p style="font-weight:bold; font-size:large">{{ctrl.offer.productName}}</p></td><td><div class="text-center" ng-bind-html="ctrl.offer.safeProductDescription"></div></td><td class="text-right">{{ctrl.offer.price | currency}}</td></tr><tr><td class="text-right" colspan="2"><strong>Zwischensumme:</strong></td><td class="text-right">{{ctrl.offer.price | currency}}</td></tr><tr><td class="text-right" colspan="2">enthalten 19% MwSt.:</td><td class="text-right">{{ctrl.offer.price * 0.19 | currency}}</td></tr><tr><td class="text-right lead" colspan="2"><strong>Gesamtpreis inkl. MwSt.:</strong></td><td class="text-right lead"><strong>{{ctrl.offer.price | currency}}</strong></td></tr></table><div ng-show="ctrl.submitted && !ctrl.accepted" class="text-danger"><strong>Bitte stimmen Sie unseren AGBs zu (siehe oben)!</strong></div><button type="button" class="btn btn-success btn-block btn-lg" ng-click="ctrl.submit()">Jetzt verbindlich buchen</button></div></div></div></div><div class="row ng-cloak" ng-show="ctrl.redirecting"><div class="col-sm-6 col-sm-offset-3"><div class="panel panel-default"><div class="panel-body text-center"><p class="lead">Ihre Anfrage wird bearbeitet. Bitte warten...</p><img src="/Content/img/bigLoader.gif" style="margin-bottom:20px;"><div><small>Bitte dr&uuml;cken Sie nicht den Zur&uuml;ck-Knopf in Ihrem Browser</small></div></div></div></div></div>');
}]);
})();
...
module.run(['$templateCache', function($templateCache) {
  $templateCache.put('/views/SpecialRequest.html',
    '<div class="row"><div class="col-xs-12 col-lg-8 col-lg-offset-2"><div class="panel panel-default"><div class="panel-heading"><h1 class="panel-title">Haben Sie spezielle W&uuml;nsche?</h1></div><div class="panel-body"><p class="lead">Kein Problem! Sagen Sie uns, wie wir helfen k&ouml;nnen.</p><div class="well well-sm"><h4>Reisedaten <small><a href="#/request?r={{ctrl.requestId}}&s=o"><span class="fa fa-pencil"></span> Bearbeiten</a></small></h4><div class="row"><div class="col-xs-2">Fahrt:</div><div class="col-xs-10">Von <strong>{{ctrl.request.startName}}</strong> nach <strong>{{ctrl.request.targetName}}</strong></div><div class="col-xs-offset-2 col-xs-10" ng-show="ctrl.request.busShouldStay"><i class="fa fa-info-circle"></i> Bus bleibt vor Ort</div><div class="col-xs-2">Hinfahrt:</div><div class="col-xs-10">Am <strong>{{ctrl.request.startDate | date}}</strong>, <span ng-show="ctrl.request.startTimeMode == \'Departure\'">Abfahrt <strong>{{ctrl.request.startTimeDepart}}</strong> (Ankunft ca. {{ctrl.request.startTimeArrive}})</span> <span ng-show="ctrl.request.startTimeMode == \'Arrival\'">Ankunft <strong>{{ctrl.request.startTimeArrive}}</strong> (Abfahrt ca. {{ctrl.request.startTimeDepart}})</span></div><div class="col-xs-2">R&uuml;ckfahrt:</div><div class="col-xs-10"><span ng-show="ctrl.request.tripType == \'OneWay\'"><strong>Keine R&uuml;ckfahrt</strong></span> <span ng-show="ctrl.request.tripType == \'Return\'">Am <strong>{{ctrl.request.returnDate | date}}</strong>, <span ng-show="ctrl.request.returnTimeMode == \'Departure\'">R&uuml;ckfahrt <strong>{{ctrl.request.returnTimeDepart}}</strong> (R&uuml;ckkunft ca. {{ctrl.request.returnTimeArrive}})</span> <span ng-show="ctrl.request.returnTimeMode == \'Arrival\'">R&uuml;ckkunft <strong>{{ctrl.request.returnTimeArrive}}</strong> (R&uuml;ckfahrt ca. {{ctrl.request.returnTimeDepart}})</span></span></div><div class="col-xs-2">Personen:</div><div class="col-xs-10"><strong>{{ctrl.request.passengers}}</strong></div></div></div><form class="form-horizontal" name="form" ng-hide="ctrl.success"><div class="form-group"><label class="col-sm-2 control-label">Anfrage</label><div class="col-sm-10"><textarea rows="5" class="form-control" required="" ng-model="ctrl.SpecialRequests" placeholder="Ihre Anforderungen"></textarea></div></div><div class="form-group"><div class="col-sm-10 col-sm-offset-2"><button type="submit" class="btn btn-primary btn-block btn-lg" ng-click="ctrl.submit()"><span class="fa fa-envelope"></span> Anfragen</button></div></div></form><div class="alert alert-success" ng-show="ctrl.success"><span class="fa fa-check-circle"></span> Vielen Dank! Wir melden uns umgehend bei Ihnen.</div></div></div></div></div>');
}]);
})();
...
(function (pfb) {
    ...
    (function (Controllers) {
        var OrderController = (function () {
            ...
            OrderController.prototype.canSubmit = function () {
    ...
    (function (Services) {
        var BookingService = (function () {
            ...
            BookingService.prototype.book = function (data) {
                var deferred = this.qService.defer();
                this.httpService.post(PFBConfig.apiEndpoint + 'booking', data).then(function (result) {
